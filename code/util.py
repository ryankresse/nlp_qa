...
'''
def get_minibatches(data, minibatch_size, data_size, shuffle=True):
    """
...
def get_minibatches(data, minibatch_size, shuffle=True):
    ...
    Args:
        data: there are two possible values:
            - list where first element of np.arrays: first is question, second is context, third is answers
        minibatch_size: the maximum number of items in a minibatch
        shuffle: whether to randomize the order of returned data
    Returns:
        minibatches:
            - a list similar to data, but with each element only containing minibatch_size number of records
...
def softmax(x):
    ...
    if len(x.shape) > 1:
        # Matrix
        ### YOUR CODE HERE
        #You have matrix of shape (n,m)
        #You want a matrix of the same shape
        #m = x.mean(axis=1).reshape(-1, 1)
        shift = np.max(x, axis=1).reshape(-1, 1)
        #sd = x.std(axis=1).reshape(-1,1)
        stdz = (x-shift)
        expon = np.exp(stdz)
        x = expon / np.sum(expon, axis=1).reshape(-1,1)
        #raise NotImplementedError
        ### END YOUR CODE
    else:
        # Vector
        #you have a vector of shape n
        # you want to return a vector of shape n
        shift = np.max(x)
        stdz = x-shift
        expon = np.exp(stdz)
        x = expon / np.sum(expon)
        #raise NotImplementedError
        ### END YOUR CODE
